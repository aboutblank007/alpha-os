...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-accent-primary/30">
      {/* Sidebar */}
      <aside
        className="fixed left-0 top-0 z-50 h-screen border-r border-white/5 bg-[#030712]/90 backdrop-blur-xl transition-all duration-500 cubic-bezier(0.2, 0, 0, 1)"
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-20 items-center justify-between px-6">
            <div className={`flex items-center gap-3 overflow-hidden transition-all duration-500 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 w-0'}`}>
              <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent-primary to-accent-secondary shadow-[0_0_20px_rgba(99,102,241,0.25)]">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={`
                    group relative flex items-center gap-3 rounded-lg px-3.5 py-3 text-sm font-medium transition-all duration-300
                    ${isActive
                ...
                >
                  {isActive && (
                    <div className="absolute left-0 top-1/2 h-6 w-0.5 -translate-y-1/2 rounded-r-full bg-accent-primary shadow-[0_0_8px_#6366f1]"></div>
                  )}
                  <item.icon
                    size={20}
                    className={`transition-colors duration-300 ${isActive ? 'text-accent-primary' : 'group-hover:text-slate-300'}`}
                  />
          ...
          <div className="border-t border-white/5 p-4">
            <button className={`group flex w-full items-center gap-3 rounded-xl p-2 transition-all hover:bg-white/5 ${!isSidebarOpen && 'justify-center'}`}>
              <div className="relative h-9 w-9 overflow-hidden rounded-full ring-1 ring-white/10 transition-all group-hover:ring-white/20">
                ...
                <div className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full bg-accent-success ring-2 ring-[#030712]"></div>
              </div>
              <div className={`flex flex-col items-start overflow-hidden transition-all duration-500 ${isSidebarOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0 hidden'}`}>
                <span className="text-sm font-medium text-slate-200">交易员</span>
                <span className="text-[10px] uppercase tracking-wider text-slate-500">专业账户</span>
              </div>
            </button>
          </div>
        </div>
      ...
      >
        ...
        <header
          className={`
            sticky top-0 z-40 flex h-16 items-center justify-between px-8 transition-all duration-300
            ${scrolled ? 'bg-[#030712]/80 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'}
          `}
        >
          {/* Search Bar */}
          <div className="flex items-center gap-3 rounded-full bg-white/5 px-4 py-2 border border-white/5 transition-all focus-within:bg-white/10 focus-within:border-white/10 hover:bg-white/10 w-64">
